...
parser = ArgumentParser(description="Receive ADiCT data-points as JSON messages on TRAP interface, aggregate them and send them via "
                                    "TRAP interface. "
...
parser.add_argument("-S", "--send-interval", type=int, metavar="seconds", default=900,
...
aggregated_data = defaultdict(lambda: {"t1": datetime.max.isoformat(), "t2": datetime.min.isoformat(), "c": 0.0, "src": set()})
# Lock to prevent concurrent access to aggregated_data
lock = threading.Lock()
# Stop flags
stop_flag_recv = threading.Event() # used to exit the data receiving loop after a signal is received
stop_flag_send = threading.Event() # used to stop the sending thread after the receiving loop stops

def process_data_points(dp):
    for data in dp:
        key = (data["type"], data["id"], data["attr"], json.dumps(data["v"], sort_keys=True))
        with lock:
            rec = aggregated_data[key]
            rec["t1"] = min(rec["t1"], data["t1"])
            rec["t2"] = max(rec["t2"], data["t2"])

            # TODO average of confidences may be a better choice in future
            if "c" in data:
...
def sending_thread_func(trap, send_interval):
    while True:
        ...
        if args.verbose:
            print(f"{datetime.now().isoformat()}: Waiting for next interval boundary at {datetime.fromtimestamp(interval_end).isoformat()} ...", flush=True)
...
def send_aggregated_data(trap, aggregated_data_copy):
    if args.verbose:
        print(f"{datetime.now().isoformat()}: Sending {len(aggregated_data_copy)} aggregated datapoints ...", flush=True)
...
def stop_program(signum, frame):
    ...
    if args.verbose:
        print("Signal received. Going to stop the program after the cached data are sent. Press Ctrl-C again to exit immediately.", flush=True)
...
# Register signal handler on common stopping signals - it sets the "stop_flag_recv" event
...
sending_thread = threading.Thread(target=sending_thread_func, args=(trap, args.send_interval))
